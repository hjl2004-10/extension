...
function setupMessageListener() {
  ...
  // 先移除已存在的监听器，防止重复绑定
  eventSource.off(event_types.CHARACTER_MESSAGE_RENDERED);
  eventSource.off(event_types.USER_MESSAGE_RENDERED);
  ...
  eventSource.on(event_types.CHARACTER_MESSAGE_RENDERED, async (messageId) => {
    ...
    audioState.processingTimeout = setTimeout(() => {
      ...
      const message = $(`.mes[mesid="${messageId}"]`).find('.mes_text').text();
      if (!message) {
        console.log('消息内容为空');
        return;
      }
      ...
      if (textStart && textEnd) {
        ...
        // 设置了标记但没找到匹配内容，不朗读
        console.log('设置了标记但未找到匹配内容，跳过戗读');
      } else {
        // 没有设置标记，朗读全文
